...
const mountView = () => {
  activeWrapper = mount(AccountsView, {
    global: {
      stubs: {
...
describe('admin AccountsView select all filtered results', () => {
  ...
  afterEach(() => {
    // AccountTableFilters' @change fires a 300ms-debounced reload
    // (AccountsView.vue's debouncedReload) that VueUse gives no cancel
    // handle for. Unmounting flips useTableLoader's isActive guard so that
    // stray reload, if it fires after this test ends, is a no-op instead of
    // hitting a listAccounts mock a later test has since reconfigured.
