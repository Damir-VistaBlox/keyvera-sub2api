...
// resetOpenAIGrokFreeQuotaGateCache clears the shared package-level cache
// between tests using sync.Map's own concurrent-safe methods, rather than
// reassigning the variable outright. A previous test's fire-and-forget
// scheduleGrokFreeQuotaStatsRefresh goroutine (the real production pattern:
// cache warming is intentionally not awaited on the request path) can still
// be running when the next test starts; reassigning `= sync.Map{}` races
// with that goroutine's reads/writes into the old map value. Range+Delete on
// the existing map is safe for concurrent use, matching how production code
// only ever touches this variable through sync.Map's own methods and never
// reassigns it. See #29.
func resetOpenAIGrokFreeQuotaGateCache() {
...
// callCount safely reads calls -- GetAccountWindowStatsBatch writes it under
// r.mu (called from the production fire-and-forget refresh goroutine), so
// polling it from require.Eventually or after must go through the same lock
// rather than reading the field directly. See #29.
...
func TestFilterGrokFreeQuotaAccountsOnlyBlocksExplicitFreeOAuth(t *testing.T) {
	...
	resetOpenAIGrokFreeQuotaGateCache()
